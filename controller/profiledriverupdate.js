// controllers/driverUpdateController.js
...
const driverUpdateController = {
  updateDriverProfile: async (req, res) => {
    try {
      const { fname, lastname, email, route, currentPassword, newPassword, profileImage } = req.body;

      console.log('Received driver update request for email:', email);
      ...
      const hasUpdates = fname !== undefined || lastname !== undefined || route !== undefined || 
                         newPassword !== undefined || profileImage !== undefined;
      ...
      // First, get the current driver data to verify current password
      const driverResult = await db.query('SELECT * FROM drivers WHERE email = $1', [email]);
        ...
        try {
          ...
          const result = await new Promise((resolve, reject) => {
            cloudinary.uploader.upload_stream(
              { folder: 'TrackSakay/DriverProfiles' },
              (error, result) => {
                if (result) {
      ...
      let updateSql = 'UPDATE drivers SET';
      let values = [];
      let paramCount = 1;
      let setClauses = [];
      ...
      // Add lname to update if provided
      if (lastname !== undefined) {
        setClauses.push(` lastname = $${paramCount}`);
        values.push(lastname);
        paramCount++;
      }

      // Add route to update if provided
      if (route !== undefined) {
      ...
      if (imageUrl) {
        setClauses.push(` imageurl = $${paramCount}`);
...
module.exports = driverUpdateController;