...
// Password verification function (same as in login controller)
...
const profileUpdateController = {
  updateProfile: async (req, res) => {
    try {
      ...
      // Add new password to update if provided (store as HASHED)
      if (newPassword) {
        const hashedNewPassword = hashPassword(newPassword); // Hash the new password
        setClauses.push(` password = $${paramCount}`);
        values.push(hashedNewPassword); // Store as hashed password
        paramCount++;
      }
