...
class JeepneyDensityController {
            ...
            thursdayData.forEach(item => {
                ...
                if (!hourBreakdown[key]) {
                    hourBreakdown[key] = {
                        weekly_patterns: 0,
                        total_jeepneys: 0,
                        dates_analyzed: new Set(),
                        areas: []
                    };
                }
                hourBreakdown[key].weekly_patterns++;
                hourBreakdown[key].total_jeepneys += item.jeepney_count;
                item.dates_analyzed?.forEach(date => hourBreakdown[key].dates_analyzed.add(date));
                hourBreakdown[key].areas.push(`(${item.area_lat}, ${item.area_lng})`);
            });
    
            res.json({
                success: true,
                total_weekly_patterns: trainingData.length,
                thursday_weekly_patterns: thursdayData.length,
                thursday_hours_available: thursdayHours,
                thursday_hour_breakdown: hourBreakdown,
                sample_thursday_entries: thursdayData.slice(0, 5).map(item => ({
                    hour: item.hour,
                    ...
                    dates: item.dates_analyzed || ['unknown']
                }))
            });
    ...
    async analyzeCitywide(req, res) {
        try {
            ...
            console.log(`🎯 City-wide analysis requested for ${targetHour}:00 (PH Time) on ${this.getDayName(targetDay)}`);
            ...
            res.json({
                ...
                analysis: {
                    ...
                    dayName: this.getDayName(targetDay),
                    areasAnalyzed: citywideAnalysis.length,
                    weekly_patterns_used: this.predictor.trainingData ? this.predictor.trainingData.length : 0,
                    predictions: citywideAnalysis,
                    system_info: {
    ...
    async getSamplePredictions(req, res) {
        try {
            ...
            const samples = [
                ...
                { hour: 14, dayOfWeek: 3, lat: 7.076, lng: 125.616, label: 'Wednesday 2PM' }
            ];
        ...
        try {
            ...
            res.json({
                ...
                status: {
                    ...
                    training_areas: this.predictor.trainingAreas ? this.predictor.trainingAreas.length : 0,
                    weekly_patterns: this.predictor.trainingData ? this.predictor.trainingData.length : 0,
                    prediction_method: this.predictor.model ? 'weekly_pattern_neural_network' : 'statistical',
                    system_type: 'weekly_pattern_jeepney_density_ml'
                }
            });
