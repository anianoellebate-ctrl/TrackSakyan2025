...
class JeepneyDensityPredictor {
    ...
    // FIXED: Extract features with weekly pattern recognition
    extractFeatures(trips) {
        const dailyHourlyData = {};
        
        // STEP 1: Group by DATE + HOUR + LOCATION
        trips.forEach(trip => {
            const utcDate = parseISO(trip.created_at);
            const phTime = new Date(utcDate.toLocaleString("en-US", {timeZone: "Asia/Manila"}));
            const hour = phTime.getHours();
            const dayOfWeek = phTime.getDay();
            const date = phTime.toDateString(); // Get actual date
            
            const areaKey = `${Math.round(trip.start_lat * 1000)},${Math.round(trip.start_lng * 1000)}`;
            const dailyKey = `${date},${hour},${areaKey}`; // Unique per day+hour+location
            
            if (!dailyHourlyData[dailyKey]) {
                dailyHourlyData[dailyKey] = {
                    date: date,
                    hour: hour,
                    dayOfWeek: dayOfWeek,
                    lat: Math.round(trip.start_lat * 1000) / 1000,
            ...
            dailyHourlyData[dailyKey].uniqueDrivers.add(trip.driver_id);
            dailyHourlyData[dailyKey].totalTrips++;
            dailyHourlyData[dailyKey].passengerCounts.push(trip.passenger_count);
        });

        // STEP 2: Group by WEEKLY PATTERNS (dayOfWeek + hour + location)
        const weeklyPatterns = {};
        
        Object.values(dailyHourlyData).forEach(dailyData => {
            ...
            if (!weeklyPatterns[patternKey]) {
                weeklyPatterns[patternKey] = {
                    ...
                    lat: dailyData.lat,
                    lng: dailyData.lng,
                    dailyJeepneyCounts: [], // Store jeepney counts per day
                    dailyTripCounts: [],    // Store trip counts per day
                    dailyPassengerAvgs: [], // Store passenger averages per day
                    dates: []               // Track which dates contributed
                };
            }
            
            const jeepneyCount = dailyData.uniqueDrivers.size;
            const avgPassengers = dailyData.passengerCounts.reduce((a, b) => a + b, 0) / dailyData.passengerCounts.length;
            
            weeklyPatterns[patternKey].dailyJeepneyCounts.push(jeepneyCount);
            weeklyPatterns[patternKey].dailyTripCounts.push(dailyData.totalTrips);
            weeklyPatterns[patternKey].dailyPassengerAvgs.push(avgPassengers);
            weeklyPatterns[patternKey].dates.push(dailyData.date);
        });

        // STEP 3: Create ML features from weekly averages
        const features = [];
        ...
        Object.values(weeklyPatterns).forEach(pattern => {
            ...
            if (pattern.dates.length >= 2) { // Reduced to 1 for testing
                ...
                const tripsPerDriver = avgTrips / avgJeepneys;
                ...
                features.push({
                    ...
                    // Behavioral features
                    total_trips: Math.round(avgTrips),
                    avg_passengers: avgPassengers,
                    trips_per_driver: tripsPerDriver,
                    
                    // Target variable (average jeepney count across dates)
                    jeepney_count: avgJeepneys,
        ...
        console.log(`📊 Created ${features.length} weekly patterns from ${Object.keys(dailyHourlyData).length} daily entries`);
        return features;
    }

    // Extract unique areas from training data
    extractUniqueAreas(features) {
        const areas = new Map();
        ...
        console.log(`📍 Unique areas from weekly patterns: ${areas.size}`);
        return Array.from(areas.values());
    }
    ...
    // Train the ML model
    async train(trips, epochs = 100) {
        console.log('🔄 Training Jeepney Density ML Model with weekly patterns...');
        ...
        this.isTrained = true;
        console.log('✅ Jeepney Density ML Model training completed!');
        
        return history;
    }

    // Analyze all trained areas
    analyzeCitywideDensity(hour, dayOfWeek) {
        if (!this.isTrained || !this.trainingAreas) {
            throw new Error('Model not trained yet. Call train() first.');
        }

        console.log(`🧠 Analyzing ${this.trainingAreas.length} areas for ${hour}:00 on ${this.getDayName(dayOfWeek)}`);
    ...
    predict(hour, dayOfWeek, lat, lng) {
        ...
        // Only use ML if we have actual training data
        if (!this.hasTrainingDataForTime(hour, dayOfWeek)) {
            return {
                jeepney_count: 0,
                density_level: "No Training Data",
    ...
    estimateFeatures(hour, dayOfWeek, lat, lng) {
        ...
        // If no similar data, use overall data patterns
        if (this.trainingData && this.trainingData.length > 0) {
            const overallTrips = this.trainingData.reduce((sum, item) => sum + item.total_trips, 0) / this.trainingData.length;
            const overallPassengers = this.trainingData.reduce((sum, item) => sum + item.avg_passengers, 0) / this.trainingData.length;
            
            return {
                total_trips: Math.max(1, Math.round(overallTrips * 0.5)),
                avg_passengers: Math.max(0.5, overallPassengers),
                trips_per_driver: 1.0
            };
        }
    ...
    calculateConfidence(predictedCount, hour, dayOfWeek, lat, lng) {
        ...
        if (predictedCount < 0) confidence -= 0.3;
        if (predictedCount > 20) confidence -= 0.2;
